...
const mockSolicitudes: SolicitudRow[] = [
  { id: 1, sucursal: 'CEMEX Puebla Centro', tipo_permiso: 'Licencia Municipal', fecha_solicitud: '2026-03-10', vigencia_propuesta: '2027-03-10', estatus: 'Pendiente', archivo: 'licencia_municipal.pdf' },
  { id: 2, sucursal: 'CEMEX CDMX Sur', tipo_permiso: 'Licencia Estatal', fecha_solicitud: '2026-03-08', vigencia_propuesta: '2027-03-08', estatus: 'Pendiente', archivo: 'licencia_estatal.pdf' },
  { id: 3, sucursal: 'CEMEX Mérida', tipo_permiso: 'Aviso de Funcionamiento', fecha_solicitud: '2026-03-05', vigencia_propuesta: '2027-03-05', estatus: 'Aprobado', archivo: 'aviso_func.pdf' },
  { id: 4, sucursal: 'CEMEX Monterrey Norte', tipo_permiso: 'Protección Civil', fecha_solicitud: '2026-03-01', vigencia_propuesta: '2027-03-01', estatus: 'Rechazado', archivo: 'proteccion_civil.pdf' },
  { id: 5, sucursal: 'CEMEX Tuxtla', tipo_permiso: 'Uso de Suelo', fecha_solicitud: '2026-02-28', vigencia_propuesta: '2027-02-28', estatus: 'Aprobado', archivo: 'uso_suelo.pdf' },
]
...
export default function SolicitudesPage() {
  ...
  const filtered = mockSolicitudes.filter((s) => {
    return !filterEstatus || s.estatus === filterEstatus
  })

  const handleAction = (solicitud: SolicitudRow, action: 'aprobar' | 'rechazar') => {
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-blue-50/30 transition-colors">
                  ...
                  <td className="py-3 px-4 font-semibold text-slate-700">{s.sucursal}</td>
                  <td className="py-3 px-4 text-gray-500">{s.tipo_permiso}</td>
                  ...
                  <td className="py-3 px-4">
                    <button className="text-red-500 hover:text-red-600 text-[11px] font-medium flex items-center gap-1 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <Badge variant={statusToBadgeVariant(s.estatus)}>{s.estatus}</Badge>
                  </td>
          ...
          <div className="space-y-4">
            <div className={`${accion === 'aprobar' ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'} border rounded-xl p-4`}>
              <p className="text-[13px] font-medium text-slate-700">
                {selectedSolicitud.tipo_permiso} — <span className="font-semibold">{selectedSolicitud.sucursal}</span>
              </p>
              <p className="text-[11px] text-gray-400 mt-1">
                Solicitud #{selectedSolicitud.id} • Vigencia propuesta: {new Date(selectedSolicitud.vigencia_propuesta).toLocaleDateString('es-MX')}
              </p>
            </div>
