      ...
      <div ref={wrapperRef} className="relative w-full min-w-[240px] max-w-xs">
        ...
        >
          {filtered.map((t) => (
            <AnimatedFilterDropdownItem key={t.id}>
              ...
              >
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-sm bg-muted-strong text-fg-faint transition-colors group-hover:bg-brand/10 group-hover:text-brand">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="truncate text-[13px] font-semibold text-fg-strong transition-colors group-hover:text-brand">
                    {t.sucursal}
                  </p>
                  <p className="truncate text-[11px] text-fg-faint">{t.region?.nombre_region ?? 'Sin región'}</p>
                </div>
              </button>
        ...
        <AnimatedFilterDropdown open={open && !!query.trim() && filtered.length === 0}>
          <div className="p-4 text-center">
            <p className="text-[12px] text-fg-faint">No se encontraron tiendas</p>
          </div>
        </AnimatedFilterDropdown>
      </div>
    </div>
